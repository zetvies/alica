      ...
      if (slashNotesStr) {
        ...
        for (let i = 0; i < slashNotesStr.length; i++) {
          ...
          if (char === "(") {
            ...
            if (parenDepth === 0) {
              const beforeParen = slashNotesStr
                .substring(Math.max(0, i - 10), i)
                .toLowerCase();
            ...
            if (afterChordExpr && parenDepth === 0 && char === ".") {
              const nextChars = slashNotesStr.substring(
                i,
                Math.min(i + 3, slashNotesStr.length),
              );
              if (nextChars === ".i(") {
        ...
        for (const item of slashItems) {
          ...
          if (chordMatch) {
            ...
            const slashInvMatch =
              remainingForSlashChord.match(/\.i\(([^)]+)\)/i);
            if (slashInvMatch) {
            ...
            if (slashParts.length === 2) {
              ...
              if (slashRootParsed) {
                ...
                const slashOctave =
                  slashRootParsed.octave !== null ? slashRootParsed.octave : 4;

                let slashChordNotes = chordToMidiNotes(
                  slashRoot,
                  slashQuality,
                  slashOctave,
                );
                if (slashChordNotes.length > 0) {
                  ...
                  if (slashInversion > 0) {
                    slashChordNotes = applyInversion(
                      slashChordNotes,
                      slashInversion,
                    );
                  }
...
function parseArrayRandomizer(str, context = {}) {
  ...
  const scaleChordMatch = arrayStr.match(
    /^(scale\(.+?\)(?:\.q\(.+?\))?|chord\(.+?\))$/i,
  );
  if (scaleChordMatch) {
  ...
  for (let i = 0; i < arrayStr.length; i++) {
    ...
    if (char === "<") {
      ...
      if (afterChordExpr && parenDepth === 0 && char === ".") {
        const nextChars = arrayStr.substring(
          i,
          Math.min(i + 3, arrayStr.length),
        );
        if (nextChars === ".i(") {
          // Keep afterChordExpr true, we're reading .i() modifier
          currentItem += char;
    ...
    if (normItem === "bt") {
      ...
      if (mDiv && mDiv[1]) {
        ...
        if (context.bt && !isNaN(divisor) && divisor > 0 && divisor % 2 === 0) {
          result.push({
            type: "duration",
            value: Math.round(context.bt / divisor),
            token: `bt/${divisor}`,
          });
          continue;
        }
        ...
        if (
          ...
          result.push({
            type: "duration",
            value: Math.round(context.bt * multiplier),
            token: `bt*${multiplier}`,
          });
          continue;
        }
...
async function playSequence(
  ...
  if (nRangeMin !== null && nRangeMax !== null) {
    ...
    while (searchIdx < processedSequence.length) {
      ...
      if (contentEnd !== -1) {
        ...
        if (
          trimmedContent.startsWith("scale(") ||
          trimmedContent.startsWith("chord(")
        ) {
          // This is a scale or chord - expand it
          const generatedNotes = generateScaleChordNotes(
            trimmedContent,
            nRangeMin,
            nRangeMax,
          );
          ...
          if (generatedNotes.length > 0) {
            ...
            const fullMatch = processedSequence.substring(
              rOIndex,
              contentEnd + 1,
            );
    ...
    for (const replacement of replacements) {
      processedSequence =
        processedSequence.substring(0, replacement.index) +
        replacement.to +
        processedSequence.substring(
          replacement.index + replacement.from.length,
        );
    }
  }
  ...
  while (i < processedSequence.length) {
    ...
    while (modifierEnd < processedSequence.length) {
      const modMatch = processedSequence
        .substring(modifierEnd)
        .match(/^\s*\.(q|i)\([^)]*\)/);
      if (modMatch) {
  ...
  while (i < processedSequence.length) {
    ...
    console.log(
      "[EXPAND] Found chord at",
      chordIndex,
      "angleDepth:",
      angleDepth,
      "context:",
      processedSequence.substring(
        Math.max(0, chordIndex - 20),
        Math.min(processedSequence.length, chordIndex + 30),
      ),
    );
    if (angleDepth > 0) {
      ...
      console.log(
        "[EXPAND] Skipping expansion, preserving:",
        processedSequence.substring(i, j),
      );
  ...
  while (countIdx < processedSequence.length) {
    ...
    while (i < processedSequence.length) {
      ...
      if (processedSequence[i] === ".") {
        const paramMatch = processedSequence
          .substring(i + 1)
          .match(
            /^(d|v|p|c|pm|pr|pmRange|prRange|nRange|vRange|pRange|dRange|nArp|dArp|vArp|pmArp|prArp|ds)\(/,
          );
        if (paramMatch) {
  ...
  while (repeatSearchIdx < processedSequence.length) {
    ...
    if (hasDurationDivide) {
      ...
      if (durationDivideIndex >= 0) {
        ...
        const beforeDivide = processedSequence.substring(
          chunkStart,
          durationDivideIndex,
        );
        const afterDivide = processedSequence.substring(
          durationDivideIndex + 1 + String(durationDivideCount).length,
          paramEnd,
        );
      ...
      console.log(
        `[SEQ_STEP_3] Expanding %${durationDivideCount} for group ${groupId}, notePattern: ${notePattern.substring(0, 100)}...`,
      );
      ...
      for (let j = 0; j < durationDivideCount; j++) {
        ...
        if (hasDurationParam) {
          ...
          expandedNote = expandedNote.replace(
            /\.d\(([^)]+)\)/g,
            (match, durationValue) => {
          ...
          if (noteEndMatch) {
            ...
            expandedNote =
              expandedNote.substring(0, noteEnd) +
              `.d(br/${originalNoteCount}/%${groupId}:${durationDivideCount})` +
              expandedNote.substring(noteEnd);
          } else {
      ...
      console.log(
        `[SEQ_STEP_3] Created ${repeatedNotes.length} notes for group ${groupId}, first note: ${repeatedNotes[0].substring(0, 150)}...`,
      );
      ...
      if (repeatIndex >= 0) {
        // Pattern includes everything before ^ and after ^N (parameters)
        const beforeRepeat = processedSequence.substring(
          chunkStart,
          repeatIndex,
        );
        const afterRepeat = processedSequence.substring(
          repeatIndex + 1 + String(repeatCount).length,
          paramEnd,
        );
  ...
  if (newSequence) {
    ...
    console.log(
      "[SEQ_STEP_4] After % expansion, sequence length:",
      expandedSequence.length,
      "chars",
    );
    console.log(
      "[SEQ_STEP_4] First 500 chars:",
      expandedSequence.substring(0, 500),
    );
  }
  ...
  while (searchIdx < expandedSequence.length) {
    ...
    while (i < expandedSequence.length) {
      ...
      if (expandedSequence[i] === ".") {
        const paramMatch = expandedSequence
          .substring(i + 1)
          .match(
            /^(d|v|p|c|pm|pr|pmRange|prRange|nRange|vRange|pRange|dRange|nArp|dArp|vArp|pmArp|prArp)\(/,
          );
        if (paramMatch) {
          const paramName = paramMatch[1];
          const paramStart = i + 1 + paramName.length + 1; // After "paramName("
  ...
  console.log(
    "[SEQ_STEP_5] First 3 chunks:",
    chunks.slice(0, 3).map((c) => c.substring(0, 100)),
  );
  if (type === "fit") {
  ...
  console.log(
    "[SEQ_STEP_6] After probability filtering, chunks remaining:",
    chunks.length,
  );
  ...
  console.log(
    "[SEQ_STEP_6] % Group counts after filtering:",
    Array.from(percentGroupCounts.entries())
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    if (percentMatch) {
      ...
      console.log(
        `[SEQ_STEP_7] Replacing duration for chunk ${i}, group ${groupId}: originalCount=${originalNoteCount}, originalPercent=${originalPercentCount}, actualCount=${actualCount}`,
      );
      ...
      const replacePattern = new RegExp(
        `\\.d\\(br/(\\d+)/%${escapedGroupId}:(\\d+)\\)`,
        "g",
      );
      const beforeReplace = chunk;
      chunk = chunk.replace(
        replacePattern,
        `.d(br/${originalNoteCount}/${actualCount})`,
      );
      if (beforeReplace !== chunk) {
        console.log(
          `[SEQ_STEP_7] Replaced: "${beforeReplace.substring(0, 150)}" -> "${chunk.substring(0, 150)}"`,
        );
      } else {
        console.log(
          `[SEQ_STEP_7] WARNING: Replacement did not match for chunk ${i}`,
        );
      }
    }
    ...
    if (beforeCleanup !== chunk) {
      console.log(
        `[SEQ_STEP_7] Cleaned up probability params: "${beforeCleanup.substring(0, 150)}" -> "${chunk.substring(0, 150)}"`,
      );
    }
    chunks[i] = chunk;
  }
  ...
  const useSignatureNumerator =
    signatureNumeratorParam !== null
      ? signatureNumeratorParam
      : signatureNumerator;
  const useSignatureDenominator =
    signatureDenominatorParam !== null
      ? signatureDenominatorParam
      : signatureDenominator;

  const beatsPerBar = useSignatureNumerator;
  const barDurationMs =
    typeof useTempo === "number" && useTempo > 0
      ? (60000 / useTempo) * beatsPerBar
      : 500;
  ...
  console.log(
    "[SEQ_STEP_9] Timing values: barDurationMs=",
    barDurationMs,
    "ev=",
    ev,
    "bt=",
  ...
  for (let idx = 0; idx < chunks.length; idx++) {
    ...
    if (noteArgLower === "r") {
      ...
      console.log(
        "[PARSE] Extracted noteArg:",
        JSON.stringify(noteArg),
        "from chunk:",
      ...
      if (chordMatch) {
        ...
        console.log(
          "[CHORD] Direct chord detected, noteArg:",
          noteArg,
          "chordNotes:",
          directChordNotes,
        );
    ...
    const paramRegex =
      /\.(d|v|p|c|pm|pr|pmRange|prRange|nRange|vRange|pRange|dRange|nArp|dArp|vArp|pmArp|prArp|ds)\((.*?)\)/g;
    let lastIndex = 0;
    let m;
    const params = [];
    ...
    if (
      params.length === 0 ||
      params.some((p) => p.raw.includes("{") && !p.raw.match(/r\.o\{.+\}/))
    ) {
    ...
    for (const param of params) {
      ...
      if (key === "d") {
        ...
        console.log(
          `[SEQ_STEP_10] Processing duration for chunk ${idx}: raw="${raw}", norm="${norm}"`,
        );
        ...
        console.log(
          `[SEQ_STEP_10] Expression result for "${norm}":`,
          exprResult,
        );

        if (mMul && mMul[1] !== "") {
          f = parseFloat(mMul[1]);
          if (type !== "fit" && !isNaN(f) && f > 0)
            duration = Math.max(0, Math.round(defaultDurationMs * f));
        } else if (mDiv && mDiv[1] !== "") {
          f = parseFloat(mDiv[1]);
          if (type !== "fit" && !isNaN(f) && f > 0)
            duration = Math.max(0, Math.round(defaultDurationMs / f));
          ...
          if (norm.startsWith("bt") || norm.startsWith("br")) {
            if (!disallowBtBr) {
              ...
              console.log(
                `[SEQ_STEP_10] Set duration from bt/br expression: ${duration}ms (from ${norm})`,
              );
            } else {
              console.log(
                `[SEQ_STEP_10] Skipped bt/br expression (disallowBtBr=true): ${norm}`,
              );
            }
          } else {
            ...
            duration = Math.max(0, Math.round(exprResult));
            console.log(
              `[SEQ_STEP_10] Set duration from number expression: ${duration}ms (from ${norm})`,
            );
          }
        } else if (norm === "r") {
          // Random duration
          ...
          if (parsedArray) {
            durationArray = parsedArray.filter(
              (item) => item.type === "duration",
            );
        ...
        if (
          (duration !== prevDuration && duration !== null) ||
          randomizeDuration
        ) {
          hasNoteDuration = true;
        }
      ...
      if (key === "v") {
        ...
        if (rawLower === "r") {
          ...
          const parsedArray = parseArrayRandomizer(raw, { bt, br });
          if (parsedArray) {
            velocityArray = parsedArray.filter(
            ...
            if (noteArray) {
              ...
              for (const item of noteArray) {
                if (item.type === "scaleChord") {
                  ...
                  const scaleChordNotes = generateScaleChordNotes(
                    item.value,
                    minMidi,
                    maxMidi,
                  );
                  ...
                  const accidental =
                    noteName.length > 1 ? noteName.substring(1) : "";
      ...
      if (key === "pm") {
        ...
        if (norm === "r") {
          ...
          const parsedArray = parseArrayRandomizer(raw, { bt, br });
          if (parsedArray) {
            muteProbabilityArray = parsedArray.filter(
              (item) => item.type === "number",
            );
      ...
      if (key === "pr") {
        ...
        if (norm === "r") {
          ...
          const parsedArray = parseArrayRandomizer(raw, { bt, br });
          if (parsedArray) {
            removeProbabilityArray = parsedArray.filter(
              (item) => item.type === "number",
            );
    ...
    if (
      noteArray &&
      (noteArray.some((item) => item.type === "scaleChord") ||
        noteArray.some((item) => item.type === "noteName"))
    ) {
    ...
    if (!hasNoteChannel && channelOverride !== null) {
      if (Array.isArray(channelOverride)) {
        ...
        channelArray = channelOverride.map((ch) =>
          Math.max(1, Math.min(16, ch)),
        );
    ...
    if (type === "fit" && weights) {
      ...
      chunkDurationPerRepeat = Math.max(
        1,
        Math.round(barDurationMs * (perInstanceWeight / totalWeight)),
      );
    } else {
      // For beat/bar type, use explicit duration or default
    ...
    if (nArpMode !== null && noteArray) {
      ...
      console.log(
        "[ARP] Note array ordered:",
        orderedNoteArray.map((item) =>
    ...
    if (pmArpMode !== null && muteProbabilityArray) {
      orderedMuteProbabilityArray = orderArrayByArp(
        muteProbabilityArray,
        pmArpMode,
      );
    }
    if (prArpMode !== null && removeProbabilityArray) {
      orderedRemoveProbabilityArray = orderArrayByArp(
        removeProbabilityArray,
        prArpMode,
      );
    }
    ...
    for (let r = 0; r < repeatCount; r++) {
      ...
      if (isDirectChord && directChordNotes) {
        ...
        console.log(
          "[CHORD] Setting useMidiNotes from direct chord:",
          useMidiNotes,
        );
      }
      ...
      if (randomizeRemoveProbability) {
        if (
          orderedRemoveProbabilityArray &&
          orderedRemoveProbabilityArray.length > 0
        ) {
          // Use array-based selection: arp ordered or random
          if (prArpMode !== null) {
            const arpItem = getArpValue(
              orderedRemoveProbabilityArray,
              currentArpPosition,
            );
            useRemoveProbability = arpItem
              ? Math.max(0, Math.min(1, arpItem.value))
              : removeProbabilityArray[0].value;
          } else {
            const randomIndex = Math.floor(
              Math.random() * removeProbabilityArray.length,
            );
            useRemoveProbability = Math.max(
              0,
              Math.min(1, removeProbabilityArray[randomIndex].value),
            );
          }
        } else {
          // Use continuous randomization with range
          const random = Math.random();
          useRemoveProbability =
            prRange[0] + random * (prRange[1] - prRange[0]);
        ...
        if (noteArray && noteArray.length > 0) {
          ...
          if (nArpMode !== null && orderedNoteArray) {
            ...
            console.log(
              ...
              currentArpPosition,
              "Selected:",
              selectedItem.type === "chord"
                ? `chord[${selectedItem.value.join(",")}]`
                : selectedItem.value,
            );
          } else {
      ...
      if (randomizeVelocity) {
        if (orderedVelocityArray && orderedVelocityArray.length > 0) {
          ...
          if (vArpMode !== null) {
            const arpItem = getArpValue(
              orderedVelocityArray,
              currentArpPosition,
            );
            scaled = arpItem ? arpItem.value : velocityArray[0].value;
          } else {
            const randomIndex = Math.floor(
              Math.random() * velocityArray.length,
            );
      ...
      if (randomizeMuteProbability) {
        if (
          orderedMuteProbabilityArray &&
          orderedMuteProbabilityArray.length > 0
        ) {
          // Use array-based selection: arp ordered or random
          if (pmArpMode !== null) {
            const arpItem = getArpValue(
            ...
            useMuteProbability = arpItem
              ? Math.max(0, Math.min(1, arpItem.value))
              : muteProbabilityArray[0].value;
          } else {
            const randomIndex = Math.floor(
              Math.random() * muteProbabilityArray.length,
            );
            useMuteProbability = Math.max(
              0,
              Math.min(1, muteProbabilityArray[randomIndex].value),
            );
          }
        } else {
          // Use continuous randomization with range
      ...
      if (randomizeDuration) {
        ...
        if (durationArray && durationArray.length > 0) {
          console.log(
            "[DURATION_RAND] Duration array found, length:",
          ...
          for (let div = 64; div >= 2; div -= 2) {
            ...
            if (dur >= 1) {
              // Only add if duration is at least 1ms
          ...
          durationArray.forEach((item) => {
            ...
            if (token) {
              ...
              const index = uniqueCanonical.findIndex(
                (canon) => canon.token === token,
              );
              if (index !== -1) {
          ...
          console.log(
            "[DURATION_ARRAY] Array tokens:",
            durationArray.map((item) => item.token),
          );
          console.log(
            "[DURATION_ARRAY] Array values:",
            durationArray.map((item) => item.value),
          );
          console.log(
            "[DURATION_ARRAY] Canonical array length:",
            uniqueCanonical.length,
          );
          ...
          if (dRange !== null && arrayIndices.length > 0) {
            ...
            console.log("[DURATION_ARRAY] dRange:", {
              minDurationMs,
              maxDurationMs,
            });
            ...
            if (
              ...
              let minIndex = uniqueCanonical.findIndex(
                (canon) => canon.value >= minDurationMs,
              );
              // maxIndex: first index where value > maxDurationMs, then subtract 1 for <= comparison
              let maxIndex = uniqueCanonical.findIndex(
                (canon) => canon.value > maxDurationMs,
              );
              const rangeMaxIndex =
                maxIndex === -1 ? uniqueCanonical.length - 1 : maxIndex - 1;

              console.log(
                "[DURATION_ARRAY] minIndex:",
                minIndex,
                "rangeMaxIndex:",
                rangeMaxIndex,
              );
              ...
              if (minIndex !== -1 && rangeMaxIndex >= minIndex) {
                ...
                console.log("[DURATION_ARRAY] Array indices:", arrayIndices);
                console.log(
                  "[DURATION_ARRAY] Valid indices after filter:",
                  validIndices,
                );
              } else {
                validIndices = [];
          ...
          if (validIndices.length > 0) {
            ...
            if (dArpMode !== null && orderedDurationArray) {
              ...
              const arpItem = getArpValue(
                orderedDurationArray,
                currentArpPosition,
              );
              if (arpItem) {
                // Find the canonical index for this duration value
                const valueIndex = uniqueCanonical.findIndex(
                  (canon) => Math.abs(canon.value - arpItem.value) <= 1,
                );
            ...
            if (dArpMode !== null && orderedDurationArray) {
              ...
              if (arpItem) {
                ...
                selectedCanonicalIndex =
                  valueIndex !== -1 && arrayIndices.includes(valueIndex)
                    ? valueIndex
                    : arrayIndices[0];
              } else {
              ...
              const randomIndex = Math.floor(
                Math.random() * arrayIndices.length,
              );
            ...
            if (dArpMode !== null && orderedDurationArray) {
              ...
              useDurationValue = arpItem
                ? arpItem.value
                : durationArray[0].value;
            } else {
              const randomIndex = Math.floor(
                Math.random() * durationArray.length,
              );
          ...
          if (
            ...
            const uniqueDurations = [...new Set(validDurations)].sort(
              (a, b) => a - b,
            );
            ...
            if (uniqueDurations.length > 0) {
              ...
              const randomIndex = Math.floor(
                Math.random() * uniqueDurations.length,
              );
          ...
          useDurationValue = Math.round(
            minDuration + random * (maxDuration - minDuration),
          );
      ...
      let finalDuration =
        useDurationValue || useDuration || defaultDurationMs || bt;
      ...
      if (useMidiNotes.length > 0) {
        ...
        if (
          shouldBreakAfterPlay ||
          (cutoffDurationMs !== null &&
            cumulativeDurationMs >= cutoffDurationMs)
        ) {
...
async function playAutomationInSequence(
  automationStr,
  type = "fit",
  cutOff = null,
  channelOverride = null,
  ...
  while (searchIdx < automationStr.length) {
    ...
    while (i < automationStr.length) {
      // Check if we've hit the start of another chunk
      if (
        i < automationStr.length - 1 &&
        automationStr.substring(i, i + 2) === "a("
      ) {
        break;
      }
      ...
      // If we find a dot, check for parameter name (whitespace already removed)
      if (automationStr[i] === ".") {
        const paramMatch = automationStr
          .substring(i + 1)
          .match(/^(from|to|d|e|c|range)\(/);
        if (paramMatch) {
          const paramName = paramMatch[1];
          const paramStart = i + 1 + paramName.length + 1; // After "paramName("
          parenCount = 1;
  ...
  for (let idx = 0; idx < automationChunks.length; idx++) {
    ...
    const rangeStr = extractMethodValue("range");
    ...
    if (rangeStr) {
      ...
      if (parts.length >= 2) {
        ...
        if (!isNaN(rMin) && !isNaN(rMax)) {
          rangeMin = Math.min(rMin, rMax);
          rangeMax = Math.max(rMin, rMax);
        }
      }
    }
    ...
    let toValue = 127;
    if (toStr !== null) {
      if (toStr.trim().toLowerCase() === "r") {
        toValue = getRandomValue();
      } else {
        const parsed = parseFloat(toStr);
        if (!isNaN(parsed)) toValue = parsed;
      }
    }

    let fromValue = toValue;
    if (fromStr !== null) {
      if (fromStr.trim().toLowerCase() === "r") {
        fromValue = getRandomValue();
      } else {
        const parsed = parseFloat(fromStr);
        if (!isNaN(parsed)) fromValue = parsed;
      }
    }
    ...
    if (durationStr) {
      ...
      if (effectiveType === "fit" && isWeightDur && weights) {
        ...
        chunkDuration = Math.max(
          1,
          Math.round(barDurationMs * (weightedTotalForChunk / totalWeight)),
        );
      } else {
    ...
    if (
      cutoffDurationMs !== null &&
      cumulativeTime + chunkDuration > cutoffDurationMs
    ) {
    ...
    const easing =
      easingStr && validEasings.includes(easingStr.toLowerCase())
        ? easingStr.toLowerCase()
        : "linear";
  ...
  if (automations.length > 0) {
    ...
    console.log(
      `[AUTOMATION] Scheduled ${automations.length} automation(s) on channel ${automations[0].channel} with type ${effectiveType}`,
    );
  }
}
...
async function playTrack(
  ...
  if (parsed) {
    ...
    useSignatureNumerator =
      parsed.signatureNumerator !== null
        ? parsed.signatureNumerator
        : signatureNumeratorParam;
    useSignatureDenominator =
      parsed.signatureDenominator !== null
        ? parsed.signatureDenominator
        : signatureDenominatorParam;
  }
  ...
  useSignatureNumerator =
    useSignatureNumerator !== null
      ? useSignatureNumerator
      : globalSignatureNumerator;
  useSignatureDenominator =
    useSignatureDenominator !== null
      ? useSignatureDenominator
      : globalSignatureDenominator;
  ...
  if (parsed && parsed.delayStart !== null && parsed.delayStart !== undefined) {
    ...
    const trackLevelDsMatch =
      cycleStr.match(/\.ds\(([^)]+)\)/) || useCycleStr.match(/\.ds\(([^)]+)\)/);
  ...
  if (!dsValue) {
    console.log(
      `[DELAY] No delayStart found - parsed:`,
      parsed ? parsed.delayStart : "no parse",
    );
  }
  ...
  if (trackDelayStart > 0) {
    ...
    console.log(
      `[DELAY] Warning: dsValue was "${dsValue}" but trackDelayStart is 0 - delay calculation may have failed`,
    );
  }
...
function playCycle(
  ...
  const effectiveTempo =
    typeof useTempo === "number" && useTempo > 0 ? useTempo : 120; // Default to 120 BPM if not set
  ...
  if (cycleDelayStart > 0) {
    console.log(
      `[CYCLE] Scheduling cycle '${finalCycleId}' to start after ${cycleDelayStart}ms delay`,
    );
  ...
  const intervalId = setInterval(() => {
    ...
    if (!cycleEntry) {
      ...
      if (cycleEntry) {
        console.log(
          `[CYCLE] Found cycle entry by id instead of intervalId for '${finalCycleId}'`,
        );
      }
    }
    ...
    const playCycleIteration = () => {
      playTrack(
        ...
        console.error(
          `[CYCLE] Error in cycle interval for '${finalCycleId}':`,
          err,
        );
      });
    };
  ...
  if (intervalId) {
    ...
    if (existingIndex !== -1) {
      ...
      console.log(
        `[CYCLE] Replaced existing cycle '${finalCycleId}' - cleared old intervalId ${oldIntervalId}, new intervalId ${intervalId}, barDuration: ${barDurationMs}ms`,
      );
    } else {
      // Add new cycle
      activeCycle.push({
      ...
      console.log(
        `[CYCLE] Created new cycle '${finalCycleId}' with intervalId ${intervalId}, barDuration: ${barDurationMs}ms, tempo: ${effectiveTempo}, beatsPerBar: ${beatsPerBar}`,
      );
    }
  } else {
    console.error(
      `[CYCLE] Failed to create interval for cycle '${finalCycleId}'`,
    );
  }
...
const exampleQueue = [
  ...
  },
  {
    id: "cycle2",
    function: () =>
      playCycle("[n(r.o{<c4,e4,g4>,<f4,a4,c5>})^3.nArp(up)].c(3)", 80, 3, 4),
  },
];
...
function updateCycleById(
  ...
  if (existingIndex === -1) {
    console.log(
      `[UPDATE CYCLE] Cycle with id '${id}' not found in activeCycle`,
    );
    return false;
  }
  ...
  const hasDelayStart =
    parsedUpdate &&
    parsedUpdate.delayStart !== null &&
    parsedUpdate.delayStart !== undefined;
  ...
  if (hasDelayStart) {
    ...
    console.log(
      `[UPDATE CYCLE] Cycle '${id}' with ds() - cleared immediately, starting new cycle`,
    );
  ...
  console.log(
    `[UPDATE CYCLE] Update queued for cycle '${id}' - will apply after current interval completes`,
  );
  return true;
}
...
function clearAllCycles() {
  ...
  console.log(
    `[CLEAR ALL CYCLES] Successfully cleared ${count} active cycle(s)`,
  );
  return count;
}
...
async function onBarChange() {
  ...
  console.log(
    `[QUEUE] Processing ${itemsToProcess.length} item(s) concurrently`,
  );
  ...
  const promises = itemsToProcess.map(async (item) => {
    if (item && item.function) {
      try {
        ...
        console.log(
          `[QUEUE] Processing item id '${item.id}', result type: ${typeof result}, result:`,
          result,
        );
        ...
        if (
          ...
          (typeof result === "number" ||
            (typeof result === "object" && result._onTimeout))
        ) {
          ...
          if (playCycleEntry) {
            ...
            if (playCycleEntry.intervalId === result) {
              console.log(
                `[QUEUE] Cycle '${item.id}' is active with intervalId ${result} (managed by playCycle)`,
              );
            } else {
              console.warn(
                `[QUEUE] Cycle '${item.id}' intervalId mismatch! Entry has ${playCycleEntry.intervalId}, playCycle returned ${result}`,
              );
            }
          } else {
            // This shouldn't happen, but add it as fallback
            console.warn(
              `[QUEUE] Cycle '${item.id}' not found in activeCycle after playCycle call - adding fallback entry`,
            );
            activeCycle.push({
              id: item.id,
          ...
          console.log(
            `[QUEUE] Item '${item.id}' result is not a number or promise (likely a track that plays once)`,
          );
        }
...
server.listen(HTTP_PORT, () => {
  ...
  console.log(
    `[WS] WebSocket server is running on ws://localhost:${HTTP_PORT}`,
  );
});
